...
To sum up, when considering each type of visualization, it's important to consider color as well as the level its of busyness. While each and every visualization has its own benefits and drawbacks, the original graph fall short. I found that segregating the original visualization into seperate visualizations helped to highlight each aspect of the main chart without adding too much information into one single chart. I have removed the concept of date from my visualization since the information it showed was irrelevant. The colors were too distracting as they were hiding the data from the chart. I had segregated the chart to keep it simple and to the point so that the audience can understand infer the valuable information from the graph that can help them to put together a real solution.

# Limitations 

In my visualizations, firstly, I wanted to display the number of cases for all the graphs, but due to the some limitation in Flourish, I was not able to display the numbers for the smaller bars. Secondly, I wanted to keep a single label for X Axis and Y Axis, however, I was unable to implement that in Flourish. Lastly, the data that I have used in my visualization is a subset from John Hopkins COVID - 19 Case Data, as the data source with which the original visualization was created had been replaced. 
